...
func (p Bucket) makeUptokenWithAccesses(key string, accesses []string) string {
...
func (p Bucket) PutParts(ctx Context, ret interface{}, key string, accesses []string, parts []PutPart, extra *PutExtra) error {

	uploader := p.makeUploader()
	uptoken := p.makeUptokenWithAccesses(key, accesses)
	return uploader.PutParts(ctx, ret, uptoken, key, *(*[]kodocli.PutPart)(unsafe.Pointer(&parts)), (*kodocli.PutExtra)(extra))
}
