...
func testBatchMove(t *testing.T) {
	...
	_, err = bucket.BatchMove(nil, KeyPair{bkey, bnewkey1, false}, KeyPair{bnewkey1, bnewkey2, false})
	if err != nil {
		t.Fatal("bucket.BatchMove failed:", err)
	}
	defer bucket.Move(nil, bnewkey2, bkey, false)
...
func testBatchCopy(t *testing.T) {
	...
	_, err := bucket.BatchCopy(nil, KeyPair{bkey, bnewkey1, false}, KeyPair{bkey, bnewkey2, false})
	if err != nil {
		t.Fatal(err)
	}
...
func testBatchDelete(t *testing.T) {
	...
	bucket.Copy(nil, bkey, bnewkey1, false)
	bucket.Copy(nil, bkey, bnewkey2, false)
...
func testBatch(t *testing.T) {
	...
	ops := []string{
		URICopy(bucketName, bkey, bucketName, bnewkey1, false),
		URIDelete(bucketName, bkey),
		URIMove(bucketName, bnewkey1, bucketName, bkey, false),
	}
