...
// keyDest 是要移动的文件的新路径。
//
func (p Bucket) Move(ctx Context, keySrc, keyDest string, force bool) (err error) {
	return p.Conn.Call(ctx, nil, "POST", p.Conn.RSHost+URIMove(p.Name, keySrc, p.Name, keyDest, force))
}
...
// keyDest    是要移动的文件的新路径。
//
func (p Bucket) MoveEx(ctx Context, keySrc, bucketDest, keyDest string, force bool) (err error) {
	return p.Conn.Call(ctx, nil, "POST", p.Conn.RSHost+URIMove(p.Name, keySrc, bucketDest, keyDest, force))
}
...
//
func (p Bucket) Copy(ctx Context, keySrc, keyDest string, force bool) (err error) {
	return p.Conn.Call(ctx, nil, "POST", p.Conn.RSHost+URICopy(p.Name, keySrc, p.Name, keyDest, force))
}
...
func (p Bucket) BatchMove(ctx Context, entries ...KeyPair) (ret []BatchItemRet, err error) {
	...
	for i, e := range entries {
		b[i] = URIMove(p.Name, e.Src, p.Name, e.Dest, e.Force)
	}
	err = p.Conn.Batch(ctx, &ret, b)
	return
}
...
func (p Bucket) BatchCopy(ctx Context, entries ...KeyPair) (ret []BatchItemRet, err error) {
	...
	b := make([]string, len(entries))
	for i, e := range entries {
		b[i] = URICopy(p.Name, e.Src, p.Name, e.Dest, e.Force)
	}
	err = p.Conn.Batch(ctx, &ret, b)
	return
}
...
func URICopy(bucketSrc, keySrc, bucketDest, keyDest string, force bool) string {
	...
	return "/copy/" + encodeURI(bucketSrc+":"+keySrc) + "/" + encodeURI(bucketDest+":"+keyDest) + arg
}
...
func URIMove(bucketSrc, keySrc, bucketDest, keyDest string, force bool) string {
	...
	return "/move/" + encodeURI(bucketSrc+":"+keySrc) + "/" + encodeURI(bucketDest+":"+keyDest) + arg
}
